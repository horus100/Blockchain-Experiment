...
def registro_():
    ...
    if request.method == "GET":
        ...
        test.measure_resources_after("INTERFAZ REGISTRO" ,cpu_before, mem_before,lat_get)
        test.salt()
        return render_template('formregistro.html', datablockr=Signlist[::-1])

    elif request.method == "POST":
        lat_post = test.start_timer()
        cpu_before_registro, mem_before_registro = test.measure_resources_before()
        ...
        # Replicar bloque
        cant = 0
        lat_repl = test.start_timer()
        cpu_before_repl, mem_before_repl = test.measure_resources_before()
        ...
        if cant == (len(config["nodos"])-1):
            ...
            test.measure_resources_after("PROCESO REGISTRO" ,cpu_before_registro, mem_before_registro,lat_post)
            test.measure_resources_after("REPLICACION DE NODOS DE PROCESO REGISTRO" ,cpu_before_repl, mem_before_repl, lat_repl)
            return redirect(request.url)
