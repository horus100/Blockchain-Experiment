...
def Processfirma_():
    try:
        ...
        if request.method == 'POST':
            ...
            lat_post = test.start_timer()
            cpu_before_pf, mem_before_pf = test.measure_resources_before()
            ...
            if not developer:  # Si no está en modo desarrollo
                ...
                solicitud = "SI"
                lat_cf = test.start_timer()
                cpu_before_cf, mem_before_cf = test.measure_resources_before()
                if len(listN) > 1:
                    ip = Ip(listN)
                    data['ipenvio'] = ip.ip()
                ...
                if solicitud != "SI":
                    test.measure_resources_after("CONSENSO DE PROCESO FIRMA" ,cpu_before_cf, mem_before_cf,lat_cf )
            ...
            try:
                ...
                if result:
                    lat_rbpf = test.start_timer()
                    cpu_before_rbpf, mem_before_rbpf = test.measure_resources_before()
                    ...
                    test.measure_resources_after("REPLICACION DE BLOQUES DE PROCESO FIRMA" ,cpu_before_rbpf, mem_before_rbpf,lat_rbpf)
                else:
                    test.measure_resources_after("PROCESO FIRMAR ERROR" ,cpu_before_pf, mem_before_pf,lat_post )
                    test.salt()
            ...
            if mensaje:
                ...
                test.measure_resources_after("PROCESO FIRMA" ,cpu_before_pf, mem_before_pf,lat_post)
                test.salt()
